...
# Debian Guest Setup Script (termuxVnc-Final-XFCE)
...
# tigervnc-standalone-server: headless VNC server
apt install xclip tigervnc-standalone-server -y
...
# Inject fixed DISPLAY=:1 into .bashrc (VNC uses :1 by default)
if ! grep -q "export DISPLAY=:1" /root/.bashrc 2>/dev/null; then
    echo "" >> /root/.bashrc
    echo "# VNC fixed display" >> /root/.bashrc
...
while true; do
    # 0. Check/Start VNC Server (Headless display)
    if ! pgrep -x "Xtigervnc" > /dev/null 2>&1; then
        echo "[$(date)] Starting VNC server..." >> "$LOGFILE"
        # Start VNC on :1, accessible from Android apps securely
        export USER=root
        vncserver :1 -geometry 1280x720 -depth 24 >> "$LOGFILE" 2>&1
    fi
...
# ==============================================================================
# 6. Secure Headless VNC Configuration
# ==============================================================================
echo "[6/6] Configuring Secure VNC Server..."

mkdir -p /root/.vnc
chmod 700 /root/.vnc

# Set the VNC password automatically using the Claimation password
VNC_PASS="${CLAIM_PASS:-admin1234}"

# We use python to gracefully create the VNC password file without interactive prompt
python3 -c "
import os, stat
passw = '${VNC_PASS}'
if len(passw) > 8: passw = passw[:8]
with os.popen('vncpasswd -f', 'w') as p:
    p.write(passw + '\n' + passw + '\n')
" > /root/.vnc/passwd
chmod 600 /root/.vnc/passwd

# VNC startup script sets up XFCE desktop to run when VNC starts
cat <<'XSTARTUP_EOF' > /root/.vnc/xstartup
#!/bin/bash
...
echo "VNC Server installed securely."
echo "Admin visibility only using password."
