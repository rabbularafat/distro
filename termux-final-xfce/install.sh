...
# --- Password Encryption Helper (runs on host, result passed to guest) ---
encrypt_pass() {
    local pass="$1"
...
# Pre-compute encrypted password on host (where openssl is guaranteed)
ENCRYPTED_PASS=""
if [ -n "$CLAIM_PASS" ]; then
    ENCRYPTED_PASS=$(encrypt_pass "$CLAIM_PASS")
    log_debug "Password encrypted on host side."
fi
...
apt-get install -y sudo nano wget curl gnupg2 ca-certificates \
    dbus-x11 procps x11-xserver-utils xvfb xclip chromium coreutils
...
# ---- 4d. Apply Hotfix to installed app.py ----
log_step "Applying Automated Hotfixes"
...
if [ -f "$APP_PY" ]; then
    ...
    # Fix startup sync fallback
    sed -i 's/initial_ext_path = get_extension_source_path()/initial_ext_path = None/' "$APP_PY"
    log_success "Hotfixes applied."
else
    log_warn "Could not find app.py at $APP_PY. Skipping hotfix."
fi

# ---- 4e. Persist Configuration ----
log_step "Persisting Application Configuration"
mkdir -p /etc/claimation
cat > /etc/claimation/config.env << CONF_EOF
# Environment configuration generated by Termux installer v4.0
MODE="${MODE}"
DEVICE="${DEVICE}"
CLAIM_USER="${CLAIM_USER}"
CONF_EOF
log_success "Config written to /etc/claimation/config.env"

# ---- 4f. Pre-configure Claimation Profile (bypass interactive setup) ----
if [ -n "$CLAIM_USER" ]; then
    log_step "Pre-configuring Claimation Profile for '${CLAIM_USER}'"
    ...
    # Store encrypted password (pre-computed on host side)
    if [ -n "$ENCRYPTED_PASS" ]; then
        echo -n "$ENCRYPTED_PASS" > "$PROFILE_DIR/claim_pass.txt"
        log_info "Encrypted password stored."
    fi

    mkdir -p /root/.claimation
    echo "Setup by Termux distro installer v4.0" > /root/.claimation/.setup_done
...
# ---- 4g. XFCE Autostart Entry (for desktop sessions) ----
mkdir -p /root/.config/autostart
cat > /root/.config/autostart/claimation.desktop << 'AUTOSTART_EOF'
[Desktop Entry]
Type=Application
Name=Claimation
Exec=claimation run
...
export ENCRYPTED_PASS="$ENCRYPTED_PASS"
export MODE="$MODE"
