...
install_claimation() {
    ...
    if [ -f "$APP_PY" ]; then
        ...
        # One-liner with hex escapes for quotes (\x27=', \x22=") to ensure compatibility with curl | bash
        sudo python3 -c 'import sys,os,re;from pathlib import Path;p="/usr/lib/claimation/claimation/app.py";(os.path.exists(p) or sys.exit(0));c=open(p,"r").read();func="""def load_env_mode():\n    app_root=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))\n    paths=[os.path.join(app_root,".env"),os.path.join(os.getcwd(),".env"),os.path.expanduser("~/.env"),"/mnt/d/backEnd/claimation/.env","/etc/claimation/config.env"]\n    env={}\n    found=None\n    for ps in paths:\n        path=Path(ps)\n        if path.exists():\n            try:\n                with open(path,"r",encoding="utf-8-sig") as f:\n                    for l in f:\n                        if "=" in l and not l.strip().startswith("#"):\n                            k,v=l.strip().split("=",1);env[k.strip()]=v.strip().strip("\x27").strip("\x22")\n                found=ps;break\n            except:pass\n    em=(os.environ.get("CLAIM_MODE") or os.environ.get("MODE") or "").strip().strip("\x27").strip("\x22")\n    m=em or env.get("CLAIM_MODE") or env.get("MODE") or "HEADLESS"\n    return m.upper(),found""";c=re.sub(r"def load_env_mode\(\):.*?return mode.upper\(\), found_path",func,c,flags=re.DOTALL);open(p,"w").write(c)'
